...
type JVBStatistics struct {
	...
	RttAggregate                          int     `json:"rtt_aggregate" prom:"Gauge;An average value (in milliseconds) of the RTT across all streams."`
	PacketRateUpload                      int     `json:"packet_rate_upload" prom:"Gauge;The total outgoing packet rate for the video bridge in packets per second."`
	...
	StressLevel                           int     `json:"stress_level" prom:"Gauge;Stress Level reported to Jicofo by the videobridge."`
	JitterAggregate                       int     `json:"jitter_aggregate" prom:"Gauge;Experimental. An average value (in milliseconds) of the jitter calculated for incoming and outgoing streams. This hasn't been tested and it is currently not known whether the values are correct or not."`
	...
	IncomingLoss                          int     `json:"incoming_loss" prom:"Gauge;The fraction of lost incoming RTP packets. This is based on RTP sequence numbers and is relatively accurate."`
